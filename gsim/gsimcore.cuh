...
public:
	...
	__device__ void neighborQueryInit2(const FLOATn &loc, float range, iterInfo &info) const;
...
__device__ void GWorld::neighborQueryInit2(const FLOATn &agLoc, float range, iterInfo &info) const {
	unsigned int tid = threadIdx.x;
	unsigned int idx = threadIdx.x + blockIdx.x * blockDim.x;
	...
	info.cellHead.x = (int)((agLoc.x - range) / modelDevParams.CLEN_X);
	info.cellTail.x = (int)((agLoc.x + range) / modelDevParams.CLEN_X);
	info.cellHead.y = (int)((agLoc.y - range) / modelDevParams.CLEN_Y);
	info.cellTail.y = (int)((agLoc.y + range) / modelDevParams.CLEN_Y);

	if (info.cellHead.x < 0) info.cellHead.x = 0;
	if (info.cellHead.y < 0) info.cellHead.y = 0;
	if (info.cellHead.x > modelDevParams.CNO_PER_DIM) info.cellHead.x = modelDevParams.CNO_PER_DIM - 1;
	if (info.cellHead.y > modelDevParams.CNO_PER_DIM) info.cellHead.y = modelDevParams.CNO_PER_DIM - 1;
