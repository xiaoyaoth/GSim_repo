...
namespace visUtil{
	...
	__global__ void reset(uchar4 *devPtr, int numDot);
};
...
public:
	...
	static void drawFunc(){
		...
		//int numDot = vis.width * vis.height * vis.dotScale * vis.dotScale;
		//int gSize = GRID_SIZE(numDot);
		//visUtil::reset<<<gSize, BLOCK_SIZE>>>(devPtr, numDot);
...
__global__ void visUtil::reset(uchar4 *devPtr, int numDot)
{
	int idx = threadIdx.x + blockIdx.x * blockDim.x;
	if (idx < numDot) {
		devPtr[idx] = colorConfigs.white;
	}
